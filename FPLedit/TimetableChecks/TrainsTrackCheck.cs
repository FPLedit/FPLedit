...
namespace FPLedit.TimetableChecks
{
    ...
    {
        public string Display => LocalizationHelper._("Ungültige Gleisangaben");
            ...
            Parallel.ForEach(tt.Trains, tra =>
            {
                ...
                foreach (var ardep in arrdeps)
                {
                    ...
                    if (!string.IsNullOrEmpty(ardep.Value.ArrivalTrack) && !tracks.Contains(ardep.Value.ArrivalTrack))
                        result.Add(LocalizationHelper._("Ungültiges Ankunftsgleis: Zug {0} / Station {1} / Gleis \"{2}\" nicht gefunden.", tra.TName, ardep.Key.SName, ardep.Value.ArrivalTrack));
                    if (!string.IsNullOrEmpty(ardep.Value.DepartureTrack) && !tracks.Contains(ardep.Value.DepartureTrack))
                        result.Add(LocalizationHelper._("Ungültiges Abfahrtsgleis: Zug {0} / Station {1} / Gleis \"{2}\" nicht gefunden.", tra.TName, ardep.Key.SName, ardep.Value.DepartureTrack));
                    foreach (var shunt in ardep.Value.ShuntMoves)
                    {
                        if ((!string.IsNullOrEmpty(shunt.SourceTrack) && !tracks.Contains(shunt.SourceTrack)) || (!string.IsNullOrEmpty(shunt.TargetTrack) && !tracks.Contains(shunt.TargetTrack)))
                            result.Add(LocalizationHelper._("Ungültiges Rangiergleis: Zug {tra.TName} / Station {ardep.Key.SName} / Gleis nicht gefunden.", tra.TName, ardep.Key.SName));
                    }
                }
            });
            return result;
